...
describe Sapience::Logger do
  describe '.add_appender' do
    ...
    it 'adds file appender' do
      ...
      assert @appender.is_a?(Sapience::Appender::File)
      assert Sapience.appenders.include?(@appender)
      assert @appender.formatter.is_a?(Sapience::Formatters::Default)
    end
    ...
    it 'adds file appender with json format' do
      ...
      assert @appender.is_a?(Sapience::Appender::File)
      assert Sapience.appenders.include?(@appender)
      assert @appender.formatter.is_a?(Sapience::Formatters::Json)
    end
    ...
    it 'adds symbol appender' do
      ...
      assert @appender.is_a?(Sapience::Appender::Wrapper), @appender.ai
      assert Sapience.appenders.include?(@appender)
    end

    it 'adds logger wrapper appender' do
      @appender = Sapience.add_appender(logger: ::Logger.new(STDOUT))
      assert @appender.is_a?(Sapience::Appender::Wrapper)
    ...
    it 'adds logger wrapper appender with color formatter' do
      ...
      assert @appender.logger.is_a?(::Logger)
      assert Sapience.appenders.include?(@appender)
      assert @appender.formatter.is_a?(Sapience::Formatters::Color)
    end

    it 'adds appender' do
      @appender = Sapience.add_appender(appender: Sapience::Appender::File.new(io: STDOUT))
      assert @appender.is_a?(Sapience::Appender::File), @appender.ai
      assert Sapience.appenders.include?(@appender)
    end

    it 'fails to add invalid logger appender' do
      assert_raises do
        Sapience.add_appender(logger: 'blah')
      end
    end
  end
  ...
  describe '.add_appender DEPRECATED' do
    after do
      Sapience.remove_appender(@appender) if @appender
      File.delete('sample.log') if File.exist?('sample.log')
    end

    it 'adds file appender' do
      @appender = Sapience.add_appender('sample.log')
      assert @appender.is_a?(Sapience::Appender::File)
      assert Sapience.appenders.include?(@appender)
    end

    it 'adds stream appender' do
      @appender = Sapience.add_appender(STDOUT)
      assert @appender.is_a?(Sapience::Appender::File)
      assert Sapience.appenders.include?(@appender)
    end

    it 'adds appender' do
      @appender = Sapience.add_appender(Sapience::Appender::File.new(io: STDOUT))
      assert @appender.is_a?(Sapience::Appender::File), @appender.ai
      assert Sapience.appenders.include?(@appender)
    end

    it 'adds logger wrapper appender' do
      @appender = Sapience.add_appender(::Logger.new(STDOUT))
      assert @appender.is_a?(Sapience::Appender::Wrapper)
      assert @appender.logger.is_a?(::Logger)
      assert Sapience.appenders.include?(@appender)
    end
  ...
  [nil, /\ALogger/, Proc.new { |l| (/\AExclude/ =~ l.message).nil? }].each do |filter|
    describe "filter: #{filter.class.name}" do
      before do
        ...
        @file_name_reg_exp = ' logger_spec.rb:\d+'

        assert_equal [], @logger.tags
        assert_equal 65535, Sapience.backtrace_level_index
      end
      ...
      Sapience::LEVELS.each do |level|
        ...
        describe level do
          ...
          it 'logs with backtrace' do
            Sapience.stub(:backtrace_level_index, 0) do
              @logger.send(level, 'hello world', @hash) { 'Calculations' }
              Sapience.flush
      ...
      describe '.default_level' do
        ...
        it 'not log at a level below the global default' do
          assert_equal :debug, Sapience.default_level
      ...
      describe '.silence' do
        ...
        it 'not log at a level below the silence level' do
          assert_equal :info, Sapience.default_level
